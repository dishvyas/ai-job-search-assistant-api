...
def test_revision_runs_when_review_marks_output_incomplete(monkeypatch):
    ...
    output, provider, fallback, _, _ = run_agentic_workflow(SAMPLE_REQUEST)
...
def test_revision_runs_at_most_once(monkeypatch):
    ...
    output, _, _, _, _ = run_agentic_workflow(SAMPLE_REQUEST)
...
def test_agentic_workflow_completes_without_rag(monkeypatch):
    ...
    output, provider, _, _, _ = run_agentic_workflow(SAMPLE_REQUEST)
...
def test_mock_mode_is_deterministic(monkeypatch):
    ...
    out1, _, _, _, _ = run_agentic_workflow(SAMPLE_REQUEST)
    out2, _, _, _, _ = run_agentic_workflow(SAMPLE_REQUEST)
